use crate::hashing::{create_jwt, hash, verify};
use crate::structs::{Account, AppState, LoginRequest, LoginResponse, RegisterRequest};
...
pub async fn post_login(
    ...
    let found = accounts.iter().find(|&acc| acc.email == data.email);
...
mod controller_tests {
    use super::*;
    use crate::hashing::{create_jwt, hash};
    ...
    async fn test_register_user() {
        ...
        let mut app = test::init_service(
            App::new()
                ...
                .service(web::scope("/api").service(post_register)),
        )
        .await;
    ...
    async fn test_login_user() {
        ...
        let mut app = test::init_service(
            App::new()
                .app_data(inital_state.clone())
                .service(web::scope("/api").service(post_login)),
        )
        .await;
    ...
    async fn test_get_user() {
        ...
        let mut app = test::init_service(
            App::new()
                .app_data(inital_state.clone())
                .service(web::scope("/api").service(get_accounts)),
        )
        .await;
